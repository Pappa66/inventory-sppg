...
// TODO: These should be loaded from global_config at runtime, not hardcoded
...
export function convertToGrams(quantity, unit) {
  ...
  if (u === "liter" || u === "l") return quantity * 1000;
  if (u === "ml") return quantity;
  if (u === "g") return quantity;
  return quantity;
}

export function nutritionFactor(quantity, unit) {
  return convertToGrams(quantity, unit) / 100;
}
