...
function header(doc, title, subtitle, logo) {
  ...
  if (logo) {
    try { doc.addImage(logo, "JPEG", 172, 2, 28, 0); } catch { try { doc.addImage(logo, "PNG", 172, 2, 28, 0); } catch {} }
  }
  doc.text("SPPG · MBG", 14, 12);
  doc.setFontSize(9);
  doc.setFont("helvetica", "normal");
  doc.text("Paket Pertanggungjawaban BPK · Inventory & Procurement", 14, 18);
...
export async function generateBpkPackage({ user, financial, lowStock, zoneStock, audit, menus, purchases, logo }) {
  ...
  doc.setFontSize(11);
  if (logo) {
    try { doc.addImage(logo, "JPEG", 170, 8, 30, 0); } catch { try { doc.addImage(logo, "PNG", 170, 8, 30, 0); } catch {} }
  }
  doc.text("SPPG · MBG", 14, 20);
  doc.setFontSize(28);
  doc.setFont("helvetica", "bold");
  doc.text("Paket Pertanggungjawaban", 14, 50);
  doc.text("BPK", 14, 62);
  doc.setFontSize(11);
  doc.setFont("helvetica", "normal");
