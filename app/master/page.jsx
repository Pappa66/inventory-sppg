        ...
        {versions && (
          <div className="fixed inset-0 z-40 bg-black/40 grid place-items-center p-4" onClick={()=>setVersions(null)}>
            <div onClick={(e)=>e.stopPropagation()} className="card-soft p-6 w-full max-w-2xl max-h-[85vh] overflow-y-auto">
              ...
              <p className="text-[#5C5C5C] text-sm">Semua perubahan tersimpan permanen. Field yang berubah ditandai.</p>
              <div className="mt-4 relative pl-6">
                <div className="absolute left-2 top-2 bottom-2 w-0.5 bg-[#EAE4D8]"/>
                {versions.rows.map((v, idx) => {
                  const prev = versions.rows[idx + 1]?.snapshot || {};
                  const cur = v.snapshot || {};
                  const fields = [
                    ["name", "Nama"],
                    ["zone", "Zona"],
                    ["category", "Kategori"],
                  ...
                  return (
                    <div key={v.id} className="mb-5 relative">
                      <div className="absolute -left-[18px] top-1 w-3 h-3 rounded-full" style={{background: v.action==="CREATE"?"#4A7C59":"#D97706"}}/>
                      <div className="flex items-baseline gap-2 flex-wrap">
                        <span className="audit-ts text-xs text-[#5C5C5C]">{fmtDateTime(v.ts)}</span>
                        <span className="tag" style={{background: v.action==="CREATE"?"#4A7C59"+"1A":"#D97706"+"1A", color: v.action==="CREATE"?"#4A7C59":"#D97706"}}>{v.action}</span>
                      ...
                      <div className="mt-2 rounded-md border border-[#EAE4D8] overflow-hidden">
                        <table className="w-full text-sm">
                          <tbody>
                            {fields.map(([k, label]) => {
                              const changed = idx < versions.rows.length - 1 && JSON.stringify(prev[k]) !== JSON.stringify(cur[k]);
                              return (
                                <tr key={k} className={`border-b border-[#EAE4D8] last:border-0 ${changed?"bg-[#FFF7E6]":""}`}>
                                  <td className="py-1.5 px-3 text-[#5C5C5C] text-xs uppercase tracking-wider w-40">{label}</td>
                                  <td className="py-1.5 px-3 audit-ts">
                                    {changed && idx < versions.rows.length - 1 ? (
                                      <span><span className="line-through text-[#C5533B]">{fmt(prev[k])}</span> <span className="text-[#5C5C5C]">→</span> <span className="text-[#4A7C59] font-semibold">{fmt(cur[k])}</span></span>
                                    ) : (
                                      fmt(cur[k])
                                    )}
                                  </td>
